...
<nav class="navbar navbar-expand-lg fixed-top navbar-dark bg-dark" aria-label="Main navigation">
  <div class="container-fluid">
    <a class="navbar-brand" href="">Dashboard Client</a>
...
<main class="container">
  ...
  @foreach ($commandes as $commande)
    @if ($commande->client_id === Cookie::get('client_id'))
        <!-- <h1>Informations de la commande :</h1> -->
        <tr>
          <th scope="row">{{ $ide }}</th>
          <td><p> {{ $commande->Sujet }}</p></td>
          <td><p>{{ $commande->LieuDepart }}</p></td>
          <td><p>{{ $commande->Destination }}</p></td>
          <td> <p>{{ $commande->Date }}</p></td>
        </tr>     
    @endif
    @php
    $ide += 1;
